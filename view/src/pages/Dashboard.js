...
import BoxeNewMutual from '../utils/BoxesSipMutual';
...
export default function Dashboard (){
    ...
    return(
        ...
        {data?<h2 style={{marginTop: '-4vh', color: 'white'}}> SBI Life</h2>:''}
         {data? <h2 style={{marginTop: '-4vh', marginLeft: '7.9vw',color: 'white', cursor: 'pointer'}} 
         onClick={()=>{setFund(false); dispatch({type: 'SWITCH_FALSE'});}} > 
         SBI Mutual 
         </h2>:
         <h2 style={{marginTop: '-4vh', marginLeft: '1vw',color: 'white', cursor: 'pointer'}} 
         onClick={()=>{setFund(false); dispatch({type: 'SWITCH_FALSE'});}} > SBI Mutual ({mutual.data?mutual.data.count: ''}) 
         <p style={{fontSize: 'calc(0.75vh + 0.75vw)', marginBottom: '-2vh',marginTop: '-0.1vh', fontWeight:'normal', color:'grey' }}>
           SIP Investment</p></h2>}
        </div>
